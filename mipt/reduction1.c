...
{
    int i, j, step,k;
    double* y = (double *)calloc(N + 1, sizeof(double)); /* сеточное решение */
    double* dy = (double *)calloc(N + 1, sizeof(double)); /* разность y^n-y^n+1 двух соседних приближений по итерациям метода Ньютона */
